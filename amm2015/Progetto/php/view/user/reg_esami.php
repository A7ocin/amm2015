                ...
                <tr <?= $i % 2 == 0 ? 'class="alt-row"' : '' ?>>
                    ...
                    <td>
                        <ul class="none no-space">
                            <?php
                            foreach ($elenco->getTemplate()->getCommissione() as $artist) {
                                echo '<li>' . $artist->getNome() . ' ' . $artist->getCognome() . '</li>';
                            }
                            ?>
                        </ul>
                    </td>
                    <td>
                        <a href="artist/reg_esami_step1?elenco=<?= $elenco->getId() ?><?= $vd->scriviToken('&')?>" title="Modifica l'elenco">
                            <img  src="../images/edit-action.png" alt="Modifica">
                        </a>
                    </td>
                    <td>
                        <a href="artist/reg_esami?cmd=r_del_elenco&elenco=<?= $elenco->getId() ?><?= $vd->scriviToken('&')?>" title="Elimina l'elenco">
...
<div class="input-form">
    <form method="get" action="artist/reg_esami_step1">
