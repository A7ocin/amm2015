...
<div class="input-form">
    <form action="artist/reg_esami_step1?elenco=<?= $elenco_id ?><?= $vd->scriviToken('&')?>" method="get">
...
if (count($sel_esami) == 0) {
        ...
        <tbody>
            ...
            foreach ($sel_esami as $statino) {
                ...
                <tr <?= $i % 2 == 0 ? 'class="alt-row"' : '' ?>>
                    ...
                    <td>
                        <a href="artist/reg_esami_step3?elenco=<?= $elenco_id ?>&index=<?= $i ?>&cmd=r_del_esame<?= $vd->scriviToken('&')?>" title="Elimina lo statino">
    ...
    <form method="get" action="artist/reg_esami_step3">
