...
<div class="input-form">
            ...
            <tbody>
                ...
                foreach ($commissione as $membro) {
                    ...
                    <tr <?= $i % 2 == 0 ? 'class="alt-row"' : '' ?>>
                        ...
                        <td>
                            <a href="artist/reg_esami_step2?elenco=<?= $elenco_id ?>&index=<?= $i ?>&cmd=r_del_commissione<?= $vd->scriviToken('&')?>" title="Elimina l'appello">
    ...
    <?php } ?>
    <form method="post" action="artist/reg_esami_step2?elenco=<?= $elenco_id ?><?= $vd->scriviToken('&')?>">

        <label for="nuovo-membro">Nuovo Membro</label>
        <select name="nuovo-membro" id="nuovo-membro">
            <?php
            $i = 0;
            foreach ($docenti as $artist) {
                ?>
                <option value="<?= $artist->getId() ?>"><?= $artist->getCognome() ?> <?= $artist->getNome() ?></option>
                <?php
                $i++;
            }
            ?>
...
<div >
    <form method="post" action="artist/reg_esami_step3?elenco=<?= $elenco_id ?><?= $vd->scriviToken('&')?>">
        <button class="avanti" type="submit" name="cmd" value="r_save_commissione">Avanti</button>
    </form>
    <form method="get" action="artist/reg_esami" class="inline">
